...
| 导出所选范围 | 在功能页勾选功能 | 按当前结果版本编译产物 | 打开结果目录 | 页面显示失败原因并保留选择 |

## Async and resilience

- Harness 进程由 Electron 主进程持有；渲染进程不能直接启动程序或访问文件系统。
- 任务级调整输入在概览、功能与需求、全部需求和待处理事项标签之间共享；切换标签不得清空草稿。提交支持按钮和 Ctrl+Enter，空输入不创建任务。
...
业务结果默认显示四个阅读入口，来源处置只保留为 Excel 隐藏追溯表。机器整理与人工确认分开；待处理事项只显示仍需动作的业务澄清和平台问题。需求详情抽屉为非模态补充阅读区域，支持 Escape 关闭。桌面本地阅读的筛选与交付范围暂存于页面组件，不写入 URL。
