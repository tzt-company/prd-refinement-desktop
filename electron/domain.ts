...
const clarificationFrom = (item:Record<string,unknown>,index:number,units:SourceUnit[],knownAffected:Set<string>):Clarification => {
  const path=`clarifications[${index}]`,state=text(item.state,`${path}.state`);if(state!=='open')throw new Error(`${path} 模型不得自动解决待确认事项`);
  const level=text(item.level,`${path}.level`);if(!clarificationLevels.has(level))throw new Error(`${path}.level 仅允许 blocking、suggestion 或 ignorable`);
  ...
  if(question.length<8||/^(NULL|统一处理[。.]?|请人工确认该原文应如何整理为需求明细[。.]?)$/i.test(question))throw new Error(`${path}.question 必须是用户可直接理解和回答的完整业务问题`);
...
export function acceptDirectFeatureBatch(featuresValue:unknown,dispositionsValue:unknown,sourceUnits:SourceUnit[]){
  ...
  const dispositions=dispositionsValue.map((raw,index)=>{const item=raw as Record<string,unknown>,contentRole=text(item.contentRole,`sourceDispositions[${index}].contentRole`);if(!dispositionKinds.has(contentRole))throw new Error(`sourceDispositions[${index}].contentRole 非法：${contentRole.slice(0,80)}；允许：${[...dispositionKinds].join("、")}`);const disposition:SourceDisposition={sourceUnitId:text(item.sourceUnitId,`sourceDispositions[${index}].sourceUnitId`),kind:contentRole as SourceDisposition['kind'],reason:text(item.reason,`sourceDispositions[${index}].reason`),featureIds:strings(item.featureIds,`sourceDispositions[${index}].featureIds`)};refs([disposition.sourceUnitId],sourceIds,`sourceDispositions[${index}].sourceUnitId`);refs(disposition.featureIds,featureIds,`sourceDispositions[${index}].featureIds`);return disposition});
...
export function acceptDirectDetails(requirementsValue:unknown,questionsValue:unknown,sourceUnits:SourceUnit[]){
  ...
  const requirements=requirementsValue.map((raw,index)=>{const item=raw as Record<string,unknown>,state=text(item.state,`requirements[${index}].state`);if(!reviewStates.has(state))throw new Error(`requirements[${index}].state 非法`);const requirement:RequirementDetail={id:text(item.id,`requirements[${index}].id`),title:text(item.title,`requirements[${index}].title`),behavior:text(item.behavior,`requirements[${index}].behavior`),conditions:strings(item.conditions,`requirements[${index}].conditions`),constraints:strings(item.constraints,`requirements[${index}].constraints`),explicitAcceptanceConditions:strings(item.explicitAcceptanceConditions,`requirements[${index}].explicitAcceptanceConditions`),sourceUnitIds:strings(item.sourceUnitIds,`requirements[${index}].sourceUnitIds`,false),ruleIds:[],state:state as RequirementDetail['state']};refs(requirement.sourceUnitIds,sourceIds,`requirements[${index}].sourceUnitIds`);
    const all=requirement.sourceUnitIds.map(sourceUnitId=>({sourceUnitId}));const rawBindings=item.evidenceBindings;
    if(rawBindings!==undefined){if(!rawBindings||typeof rawBindings!=='object'||Array.isArray(rawBindings))throw new Error(`${requirement.id}.evidenceBindings 必须是对象`);const binding=rawBindings as Record<string,unknown>,one=(value:unknown,label:string)=>{if(Array.isArray(value))return sourceRefs({sourceRefs:value},sourceUnits,label);if(value&&typeof value==='object'){const entry=value as Record<string,unknown>;return sourceRefs(Array.isArray(entry.sourceRefs)?entry:{sourceRefs:[entry]},sourceUnits,label)}return sourceRefs({sourceUnitIds:value},sourceUnits,label)},many=(value:unknown,count:number,label:string)=>{if(!Array.isArray(value)||value.length!==count)throw new Error(`${label} 必须与对应字段逐项匹配`);return value.map((entry,n)=>one(entry,`${label}[${n}]`))};requirement.evidenceBindings={behavior:one(binding.behavior,`${requirement.id}.evidenceBindings.behavior`),conditions:many(binding.conditions,requirement.conditions.length,`${requirement.id}.evidenceBindings.conditions`),constraints:many(binding.constraints,requirement.constraints.length,`${requirement.id}.evidenceBindings.constraints`),explicitAcceptanceConditions:many(binding.explicitAcceptanceConditions,requirement.explicitAcceptanceConditions.length,`${requirement.id}.evidenceBindings.explicitAcceptanceConditions`)}}
    ...
    const bound=[...requirement.evidenceBindings.behavior,...requirement.evidenceBindings.conditions.flat(),...requirement.evidenceBindings.constraints.flat(),...requirement.evidenceBindings.explicitAcceptanceConditions.flat()];const derived=[...new Set(bound.map(ref=>ref.sourceUnitId))];if(derived.some(id=>!requirement.sourceUnitIds.includes(id)))throw new Error(`${requirement.id}.evidenceBindings 超出需求来源范围`);return requirement});uniqueIds(requirements,'requirements');
...
export function acceptRequirementRelations(value:unknown,sourceUnits:SourceUnit[],requirements:RequirementDetail[]):RequirementRelation[]{
  if(value===undefined)return[];if(!Array.isArray(value))throw new Error('relations 必须是数组');const ids=new Set(requirements.map(item=>item.id)),kinds=new Set(['depends-on','affects','exception-to']);
  const relations=value.map((raw,index)=>{if(!raw||typeof raw!=='object'||Array.isArray(raw))throw new Error(`relations[${index}] 必须是对象`);const item=raw as Record<string,unknown>,sourceRequirementId=text(item.sourceRequirementId,`relations[${index}].sourceRequirementId`),targetRequirementId=text(item.targetRequirementId,`relations[${index}].targetRequirementId`),kind=text(item.kind,`relations[${index}].kind`);refs([sourceRequirementId,targetRequirementId],ids,`relations[${index}]`);if(sourceRequirementId===targetRequirementId)throw new Error('需求关系禁止自引用');if(!kinds.has(kind))throw new Error(`relations[${index}].kind 非法`);const selected=sourceRefs(item,sourceUnits,`relations[${index}]`);return{id:typeof item.id==='string'&&item.id.trim()?item.id.trim():`LOCAL-REL-${index+1}`,sourceRequirementId,targetRequirementId,kind:kind as RequirementRelation['kind'],sourceRefs:selected}});
