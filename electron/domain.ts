...
export function acceptDirectDetails(requirementsValue:unknown,questionsValue:unknown,sourceUnits:SourceUnit[]){
  ...
  const normalize=(value:string)=>value.replace(/\s/g,'');for(const requirement of requirements){const originals=sourceUnits.filter(unit=>requirement.sourceUnitIds.includes(unit.id)).map(unit=>normalize([unit.excerpt,unit.context??'',unit.asset?.extractedText??''].join('\n')));const unsupported=requirement.explicitAcceptanceConditions.filter(condition=>!originals.some(original=>original.includes(normalize(condition))));if(unsupported.length)throw new Error(`${requirement.id}.explicitAcceptanceConditions 必须逐字引用关联原文，不得推导或静默丢弃：${unsupported.join('；')}`)}
