...
export class AnalysisTaskScheduler {
  ...
  async initialize() {
    ...
    for (const file of await readdir(this.root)) {
      ...
      if (task.checkpoint?.pipelineVersion !== 13 && !['completed','needs-attention'].includes(task.status)) {
  ...
  async retry(id: string) {
    ...
    if (task.checkpoint?.pipelineVersion !== 13) throw new Error('旧版检查点不可续跑，请使用任务保存的原始材料重新执行');
    ...
    try {
      ...
      while (true) {
        await stage(4, async () => {
          ...
          const supplements = await mapPool(targets, pool, async ({ feature, units }) => {
            ...
            const partials=await mapPool(batches(units,6,5000),pool,async(batch,index)=>call('details',`details-repair-${feature.id}-batch${index}`,'逐功能细化·定点补漏',`仅输出本批缺失原文要求的新条目或真实待澄清事项，不重复已有条目。existingRequirements 仅用于按标题和已承接来源避免语义重复。每个字段的 evidenceBindings 必须只选择直接支持该字段的证据。${clarificationContract} 输出 ${detailSchema}`,{feature:featureInput,sourceUnits:batch,existingRequirements},v=>acceptDirectDetails(v.requirements,v.clarifications,batch)));
      ...
      await stage(6, async () => {
        ...
        if(task.project.clarifications.some(item=>item.state==='open')){
          ...
          for(const action of actions.filter(item=>item.action==='remove-answered')){const removed=new Set(action.clarificationIds);task.project.clarifications=task.project.clarifications.filter(item=>!removed.has(item.id));for(const issue of cp.auditIssues)if(issue.clarificationId&&removed.has(issue.clarificationId)||issue.affectedIds.some(id=>removed.has(id)))closeIssue(issue,'repaired')}
      ...
      if(cp.auditIssues.some(issue=>issue.disposition==='open'&&issue.owner==='feature-grouping')){task.steps[5].status='pending';task.steps[6].status='pending';await checkpoint();return this.run(task,config,runtimes)}
      await stage(7, async () => {
