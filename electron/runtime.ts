...
export class CodexCliRuntime implements AnalysisRuntime {
  ...
  async start(cwd:string,config:RuntimeConfig){if(!await codexLauncher())throw new Error('未找到官方 Codex CLI');await mkdir(cwd,{recursive:true});this.cwd=cwd;this.config=config}
  async promptAndWait(sessionId:string,text:string,timeoutMs=10*60_000,images:RuntimeImage[]=[]){
    ...
    const args=['exec','--json','--ephemeral','--skip-git-repo-check','--sandbox','read-only','--ignore-user-config','--ignore-rules','--disable','apps','--disable','browser_use','--disable','computer_use','--disable','image_generation','--disable','memories','--disable','plugins','--disable','skill_search','--disable','shell_tool','--disable','unified_exec','--disable','view_image','--disable','workspace_dependencies','--disable','goals','--disable','multi_agent','-C',this.cwd,'-m',this.config.model];if(this.config.reasoningEffort!=='default')args.push('-c',`model_reasoning_effort=${JSON.stringify(this.config.reasoningEffort)}`);for(const image of images)args.push('--image',image.path);args.push('-');
    const env={...process.env};delete env.CODEX_SESSION_ID;delete env.CODEX_THREAD_ID;delete env.CODEX_PERMISSION_PROFILE;const child=spawn(launcher.bin,args,{cwd:this.cwd,env,stdio:['pipe','pipe','pipe'],windowsHide:true});this.children.add(child);child.stdin!.end(text,'utf8');
    let buffer='',answer='',settled=false,turnFailed=false,turnCompleted=false;const failures:string[]=[];
    return new Promise<string>((resolve,reject)=>{
      ...
      // 消费 stderr 防止管道阻塞；任意日志可能含提示词或凭据，不进入诊断。
      child.stderr!.on('data',()=>{});child.once('error',error=>finish(new Error(`Codex 启动失败：${redactRuntimeError(error.message,this.config?.apiKey)}`)));child.once('close',code=>{if(buffer.trim())lineEvent(buffer);const succeeded=code===0&&!turnFailed&&(turnCompleted||failures.length===0);if(succeeded)failures.length=0;finish(succeeded?undefined:new Error(`Codex 已退出（${code??'unknown'}）${failures.length?`：${failures.join('；')}`:''}`))});
    })
  }
