...
import {checksPass,closeIssue,contentFingerprint,projectDependencyHash,registerAuditIssues,requiredChecks,sourceCoverageDecisionValid} from '../electron/task-execution-state';
...
describe('pipeline 10 任务状态内核',()=>{
  ...
  it('来源承接决定在承接需求变化后自动失效',()=>{const p=project(),decision={status:'covered-by-existing' as const,issueId:'A-1',reason:'已有需求承接',requirementIds:['R-0001'],dependencyHash:projectDependencyHash(p,['R-0001'],['S-001'],false),at:1};expect(sourceCoverageDecisionValid(p,'S-001',decision)).toBe(true);p.requirements[0].behavior='字段 X 可空';expect(sourceCoverageDecisionValid(p,'S-001',decision)).toBe(false)});
});
