...
it('直接引用原文的待确认显示在相关功能和需求行，不扩散到同功能其他来源',async()=>{
  ...
  expect(workbook.getWorksheet('功能清单')!.getCell('C2').value).toEqual(expected);
...
it('导出明确的约束适用关系，并保留直接引用原文的待确认来源',async()=>{
  ...
  expect(features.getCell('F1').value).toBe('条目类型');expect(features.getCell('G1').value).toBe('适用功能');
  expect(features.getCell('F2').value).toBe('业务功能');expect(features.getCell('F4').value).toBe('跨功能约束');
  expect(features.getCell('G4').value).toBe('F-001');expect(features.getCell('G4').value).not.toContain('F-002');
...
it('多文件同名标题在主表与追溯中保留独立路径、版本及资料角色',async()=>{
  ...
  const workbook=new ExcelJS.Workbook();await workbook.xlsx.readFile(file);
  const main=workbook.getWorksheet('功能清单')!.getCell('E2').value;
