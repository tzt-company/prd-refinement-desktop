...
function answer(prompt:string){
  ...
  if(prompt.includes('“待澄清事项跨批一致性检查”'))return{actions:value.clarifications.map((item:{id:string})=>({action:'keep',clarificationIds:[item.id],reason:'不同业务决定'}))};
  if(prompt.includes('“待澄清事项证据分片汇总”'))return{selectedIndex:-1,reason:'分片证据仍不足以得到唯一答案'};
...
function broadClarificationDetails(p:string,singleQuestion:boolean){
  const value=input(p),units=value.sourceUnits as SourceUnit[],catalog=value.evidenceCatalog as Array<{id:string;sourceUnitId:string}>;
  return{requirements:units.map((unit,index)=>({id:`LOCAL-R${index}`,title:`字段 ${index}`,behavior:'校验条件和维护规则'.repeat(singleQuestion?180:75),conditions:[],constraints:[],explicitAcceptanceConditions:[],sourceUnitIds:[unit.id],evidenceBindings:{behavior:catalog.filter(item=>item.sourceUnitId===unit.id).map(item=>item.id),conditions:[],constraints:[],explicitAcceptanceConditions:[]},state:'draft'})),clarifications:singleQuestion?[clarification(units[0].id,units.map(unit=>unit.id))]:units.map((unit,index)=>clarification(unit.id,[unit.id],{id:`LOCAL-Q${index}`}))};
}
const broadPrd=(count:number)=>project(Array.from({length:count},(_,index)=>`字段 ${index} 需要校验和维护。`).join('\n\n'));
...
describe('八节点需求细化调度器',()=>{
  ...
  it('跨批相同业务澄清统一合并，保留全部来源和历史编号',async()=>{
    let batches=0,cross=0;const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”'))return broadClarificationDetails(p,false);if(p.includes('“待澄清事项全局有效性与一致性检查”'))batches++;if(p.includes('“待澄清事项跨批一致性检查”')){cross++;return{actions:[{action:'merge',clarificationIds:v.clarifications.map((q:{id:string})=>q.id),reason:'所有来源都询问同一个字段空值决定'}]}}return answer(p)}));
    await s.initialize();await s.create(broadPrd(35));const done=await terminal(s);expect(done.status,done.error).toBe('needs-attention');expect(batches).toBeGreaterThan(1);expect(cross).toBeGreaterThan(0);expect(done.project.clarifications).toHaveLength(1);expect(done.project.clarifications[0].aliases).toHaveLength(34);expect(done.project.clarifications[0].affectedIds).toHaveLength(35);expect(done.project.clarifications[0].sourceRefs).toHaveLength(35);
  });
  it('单个澄清的十二条长需求完整分片且不再超限',async()=>{
    const seen=new Set<string>(),sources=new Set<string>();let batches=0;const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”'))return broadClarificationDetails(p,true);if(p.includes('“待澄清事项全局有效性与一致性检查”')){batches++;for(const r of v.requirements)seen.add(r.id);for(const u of v.sourceUnits)sources.add(u.id);expect(v.clarifications).toHaveLength(1)}return answer(p)}));
    await s.initialize();await s.create(broadPrd(12));const done=await terminal(s);expect(done.status,done.error).toBe('needs-attention');expect(batches).toBeGreaterThan(1);expect(seen.size).toBe(12);expect(sources.size).toBe(12);expect(done.project.clarifications).toHaveLength(1);expect(done.checkpoint?.promptMetrics?.every(item=>item.estimatedTokens<=item.hardTokens)).toBe(true);
  });
  it('多组跨批核对覆盖每一对问题，同名但不同业务保持独立',async()=>{
    const pairs=new Set<string>();let comparisons=0;const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”')){const result=broadClarificationDetails(p,false);result.clarifications=result.clarifications.map(q=>({...q,knownFacts:`独立业务对象 ${q.affectedIds[0]}；${'各自配置独立管理'.repeat(60)}`}));return result}if(p.includes('“待澄清事项跨批一致性检查”')){comparisons++;for(let left=0;left<v.clarifications.length;left++)for(let right=left+1;right<v.clarifications.length;right++)pairs.add([v.clarifications[left].id,v.clarifications[right].id].sort().join('|'));expect(v.clarifications.every((q:{knownFacts:string})=>q.knownFacts.includes('独立业务对象'))).toBe(true)}return answer(p)}));
    await s.initialize();await s.create(broadPrd(20));const done=await terminal(s);expect(done.status,done.error).toBe('needs-attention');expect(comparisons).toBeGreaterThan(1);expect(pairs.size).toBe(20*19/2);expect(done.project.clarifications).toHaveLength(20);
  });
  it.each([false,true])('分片删除候选结合全部理由后裁决，冲突不误删（冲突=%s）',async conflict=>{
    let summaries=0;const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”'))return broadClarificationDetails(p,true);if(p.includes('“待澄清事项全局有效性与一致性检查”')){const answered=v.requirements.some((r:{id:string})=>r.id==='R-0001');return{actions:[{action:answered?'remove-answered':'keep',clarificationIds:[v.clarifications[0].id],satisfiedRequirementIds:answered?['R-0001']:[],reason:answered?'第一片明确空值禁止提交':conflict?'另一片明确空值允许提交，与第一片冲突':'当前片仅描述其他字段，无新增答案'}]}}if(p.includes('“待澄清事项证据分片汇总”')){summaries++;expect(v.evidenceDecisions.length).toBeGreaterThan(1);expect(v.evidenceDecisions.some((d:{action:string})=>d.action==='keep')).toBe(true);return{selectedIndex:conflict?-1:v.evidenceDecisions.findIndex((d:{action:string})=>d.action==='remove-answered'),reason:conflict?'允许与禁止冲突，不能认定已有唯一答案':'其他片未否定首片明确答案'}}return answer(p)}));
    await s.initialize();await s.create(broadPrd(12));const done=await terminal(s);expect(done.status,done.error).toBe(conflict?'needs-attention':'completed');expect(summaries).toBe(1);expect(done.project.clarifications).toHaveLength(conflict?1:0);
  });
  it('跨批检查失败不提前提交，重试复用已经完成的有效性分片',async()=>{
    let validity=0,cross=0;const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”'))return broadClarificationDetails(p,false);if(p.includes('“待澄清事项全局有效性与一致性检查”')){validity++;return{actions:[{action:'merge',clarificationIds:v.clarifications.map((q:{id:string})=>q.id),reason:'同一业务决定'}]}}if(p.includes('“待澄清事项跨批一致性检查”')){cross++;if(cross===1)throw new Error('跨批 Runtime 临时故障');return{actions:[{action:'merge',clarificationIds:v.clarifications.map((q:{id:string})=>q.id),reason:'同一业务决定'}]}}return answer(p)}));
    await s.initialize();const created=await s.create(broadPrd(35));const first=await terminal(s),before=validity;expect(first.status).toBe('failed');expect(first.project.clarifications).toHaveLength(35);expect(Object.keys(first.checkpoint?.clarificationResults?.results??{}).length).toBeGreaterThan(1);await s.retry(created.id);const done=await terminal(s);expect(done.status,done.error).toBe('needs-attention');expect(validity).toBe(before);expect(cross).toBe(2);expect(done.project.clarifications).toHaveLength(1);
  });
  it('跨批检查期间取消不会提交批内合并',async()=>{
    const entered=deferred(),release=deferred();const s=new AnalysisTaskScheduler(caseRoot(),async()=>config,()=>{},()=>runtime(p=>{const v=input(p);if(p.includes('“逐功能细化”'))return broadClarificationDetails(p,false);if(p.includes('“待澄清事项全局有效性与一致性检查”'))return{actions:[{action:'merge',clarificationIds:v.clarifications.map((q:{id:string})=>q.id),reason:'同一决定'}]};return answer(p)},async p=>{if(p.includes('“待澄清事项跨批一致性检查”')){entered.resolve();await release.promise}}));
    await s.initialize();const created=await s.create(broadPrd(35));await entered.promise;await s.cancel(created.id);release.resolve();const done=await terminal(s);expect(done.error).toBe('用户已取消任务');expect(done.project.clarifications).toHaveLength(35);expect(done.project.delivery?.state).not.toBe('ready');
  });
