...
export function MaterialWorkspace({ onStart, onBack }: { onStart: (project: PrdProject) => Promise<void>; onBack: () => void }) {
  ...
  return <div className={`page material-page ${dragging ? 'material-dragging' : ''}`}
    ...
    <div className={`material-page-heading ${bundle ? 'is-compact' : ''}`}><span>{bundle ? '新建任务 · 资料准备' : '新建分析任务'}</span><h1>{bundle ? '准备分析资料' : '从主 PRD 开始'}</h1><p>{bundle ? '确认主 PRD，按需补充相关材料，然后建立索引。' : '选择已经定版的需求文档，平台会保存副本并进入资料准备。'}</p>{!bundle&&<ol className="material-steps"><li className="active"><b>1</b>选择主 PRD</li><li><b>2</b>补充资料 <small>可选</small></li><li><b>3</b>识别并分析</li></ol>}</div>
