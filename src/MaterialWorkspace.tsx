...
export function MaterialWorkspace({ onStart, onBack }: { onStart: (project: PrdProject) => Promise<void>; onBack: () => void }) {
  ...
  async function select(id: string) { if (!api || lock.current) return; setSource(undefined); setEditing(undefined); setResult(undefined); setOffset(0); setQueryFile(''); setFilePage(0); await run('读取资料包', () => api.get(id)); }
  useEffect(() => {
  ...
  return <div className={`page material-page ${dragging ? 'material-dragging' : ''}`}
    ...
    {(busy || dragging || bundle) && <div className="material-feedback" role="status" aria-live="polite"><span>{busy || (dragging ? `松开添加为${bundle ? roles[role] : '主 PRD'}` : saved || '文件与修改保存在本机，可离开后继续准备。')}</span>{busy==='添加并保存资料'&&<button className="secondary" onClick={()=>void cancelAddition()}>取消添加</button>}{busy && <RotateCw className="runtime-spinner"/>}</div>}
    ...
    {!api ? <div className="panel">资料导入需要在桌面应用中使用。</div> : <>
      {bundles.length > 0 && <label className="bundle-select">继续上次准备<select aria-label="已有资料包" disabled={!!busy} value={bundle?.id ?? ''} onChange={e => void select(e.target.value)}><option value="">选择已有资料</option>{bundles.map(item => <option key={item.id} value={item.id}>{item.name || item.id} · {states[item.state]} · v{item.revision}</option>)}</select></label>}
