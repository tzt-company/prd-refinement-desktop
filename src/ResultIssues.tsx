...
export function ResultIssues({project}:{project:PrdProject}){
  ...
  return <section className="issues-workspace">
    ...
    {visible.length===0?<div className="issue-empty"><CheckCircle2/><strong>当前没有这类待处理事项</strong><span>可以查看其他级别或返回概览。</span></div>:<div className="issue-list">{visible.map(item=>{
      ...
      const facts=item.kind==='platform'?item.value.detail:legacy?'当前记录缺少完整的已知事实，需要平台重新分析。':clarification!.knownFacts;
      const impact=item.kind==='platform'?'修正并重新检查前不能生成正式 Agent 需求包。':legacy?'重新分析后才能形成可回答的业务问题。':clarification!.impact;
      return <article className="issue-row" key={`${item.kind}-${item.value.id}`}><span className={`issue-level ${level}`}>{item.kind==='platform'?'阻塞':clarificationLevelLabel[level]}</span><div><strong>{title}</strong><p><b>已知：</b>{facts}</p><p><b>影响：</b>{impact}</p><small>{item.kind==='platform'?'平台处理':level==='blocking'?'需要你澄清':level==='suggestion'?'建议确认':'无需决定'} · 影响 {affectedLabels(project,item.value.affectedIds).slice(0,2).join('、')}</small></div><button className="issue-open" aria-label={`查看详情：${title}`} onClick={event=>{setOpener(event.currentTarget);setSelectedKey(`${item.kind}-${item.value.id}`)}}>查看依据<ChevronRight/></button></article>
    })}</div>}
    ...
    {selected&&<IssueDrawer project={project} item={selected} opener={opener} onClose={()=>setSelectedKey(undefined)}/>}
  </section>;
}

function IssueDrawer({project,item,opener,onClose}:{project:PrdProject;item:Item;opener?:HTMLElement;onClose:()=>void}){
  const clarification=item.kind==='clarification'?item.value:undefined,legacy=clarification&&!clarification.knownFacts;const level=item.kind==='platform'?'blocking':clarificationLevel(clarification!);function close(){onClose();requestAnimationFrame(()=>opener?.focus())}useEffect(()=>{const listener=(event:KeyboardEvent)=>{if(event.key==='Escape')close()};window.addEventListener('keydown',listener);return()=>window.removeEventListener('keydown',listener)},[opener,onClose]);
  return <aside className="drawer issue-drawer" aria-label="待处理事项详情"><header><div><span className={`issue-level ${level}`}>{item.kind==='platform'?'阻塞':clarificationLevelLabel[level]}</span><h2>{item.kind==='platform'?issueTitle(item.value):legacy?'旧任务中的待确认内容需要重新分析':clarification!.question}</h2><small>{item.kind==='platform'?'处理方：平台':'处理方：需求负责人'}</small></div><button onClick={close} aria-label="关闭"><X/></button></header><div>
    ...
    {item.kind==='clarification'&&!legacy&&<p className="issue-next">可在页面上方的调整说明中一次回答这项或多项澄清，无需逐项填写。</p>}
