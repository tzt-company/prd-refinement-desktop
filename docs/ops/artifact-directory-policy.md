...
本次只制定规范。下文标注“目标”的路径尚未替换当前源码默认值；迁移未完成前，不能把目标路径写成可直接运行的现有配置，也不能仅移动目录而不修改生成入口。
...
| 编译缓存、展开包与打包临时结果（目标） | `docs/tmp/desktop-build/<run-id>/{dist,dist-electron,dist-release}/` | 否 | 构建完成后先提取发布证据；当前仍在根目录，需配套代码迁移 |
| 测试工具报告（目标） | `docs/tmp/test-run/<run-id>/{test-results,playwright-report}/` | 否 | 关键失败或通过证据归相应验收轮次 |
...
- **先收敛入口**：E2E 默认目录、测试临时根、工具报告、UI 审计输出分别改到规范落点。构建输出迁移需同步 `package.json` 的 main/files/output、TypeScript/Vite 配置、脚本导入和 Electron 启动路径，验证构建、启动与打包，不能只改 `.gitignore`。
...
当前规范靠执行约定生效，尚无自动目录门禁、自动留存或清理器。任何后续自动化需明确覆盖范围、例外与反例，不能仅以规范文件存在宣称目录已经治理完成。
