...
try{
  ...
  await window.getByRole('button',{name:'功能与需求',exact:true}).click();await expect(window.getByText('订单备注风控')).toBeVisible();await expect(window.locator('.data-table.features .thead')).toHaveText(/功能名称.*原文位置.*需求/);await expect(window.locator('.data-table.features .thead')).not.toContainText('类型');
  console.log('PASS: production Electron 展示三级待处理事项、可读功能名称和来源，移除功能类型列，并允许仅有建议项的任务交付');
