...
try {
 ...
 await mkdir('docs/acceptance/material-bundle-index/round-5',{recursive:true});
 await page.screenshot({path:'docs/acceptance/material-bundle-index/round-5/prd-entry-1440.png',fullPage:true});
 ...
 await page.screenshot({path:'docs/acceptance/material-bundle-index/round-5/material-preparation-1440.png',fullPage:true});
 await page.setViewportSize({width:1100,height:720});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),true);
 await page.screenshot({path:'docs/acceptance/material-bundle-index/round-5/material-preparation-1100.png',fullPage:true});
 ...
 await page.screenshot({path:'docs/acceptance/material-bundle-index/round-5/material-ui-1440.png',fullPage:true});
 await page.setViewportSize({width:1100,height:720});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),true);
 await page.screenshot({path:'docs/acceptance/material-bundle-index/round-5/material-ui-1100.png',fullPage:true});
 ...
 console.log('PASS: 资料包创建、目录挂载、路径保存、仅使用上传文件、索引轮询、ready门禁、分页检索、来源预览、Escape、1440/1100无横向溢出、分析交接。渲染层受控API验证。');
