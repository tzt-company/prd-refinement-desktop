...
const buildHashes=Object.fromEntries(await Promise.all(['scheduler-v2','clarification-reconciliation','clarification-relations'].map(async name=>[name,createHash('sha256').update(await readFile(new URL(`../../../../dist-electron/electron/${name}.js`,import.meta.url))).digest('hex')])));
...
function summarize(task,wave,index){
  ...
  const checks=task.checkpoint?.checks??{},checksPassed=['source','feature','detail','relation','clarification'].every(id=>checks[id]?.status==='passed'&&checks[id]?.resultVersion===task.checkpoint?.resultVersion),unverifiedScopeIds=task.project.delivery?.unverifiedScopeIds??[];
  return {wave,index,id:task.id,status:task.status,error:task.error,wallMs,within20Minutes:wallMs<=20*60_000,pipelineVersion:task.checkpoint?.pipelineVersion,deliveryState:task.project.delivery?.state,platformOpen:platformIssues.length,blockingClarifications:blocking.length,checksPassed,unverifiedScopeIds,features:task.project.features.length,requirements:task.project.requirements.length,relations:task.project.relations?.length??0,promptAttempts:prompts.length,promptEstimatedTokens:prompts.reduce((sum,item)=>sum+item.estimatedTokens,0),maxPromptEstimatedTokens:Math.max(0,...prompts.map(item=>item.estimatedTokens)),hardBudgetViolations:prompts.filter(item=>item.estimatedTokens>item.hardTokens).length,runtimeCalls:runtime.length,inputTokens:runtime.reduce((sum,item)=>sum+(item.inputTokens??0),0),cachedInputTokens:runtime.reduce((sum,item)=>sum+(item.cachedInputTokens??0),0),outputTokens:runtime.reduce((sum,item)=>sum+(item.outputTokens??0),0),sourceHash:task.project.sourceHash};
}

const results=[];
console.log(JSON.stringify({runRoot,sourceTaskId,pipelineVersion:CURRENT_PIPELINE_VERSION,buildHashes}));
...
const report={startedAt:new Date(events[0]?.at??Date.now()).toISOString(),completedAt:new Date().toISOString(),sourceTaskId,sourceHash:sourceTask.project.sourceHash,buildHashes,config:{adapter:config.adapter,provider:config.provider,fastModel:config.fastModel,model:config.model,reasoningEffort:config.reasoningEffort,maxParallel:5,maxNodeParallel:10,taskDeadline:'none'},runRoot,results,passed:results.length===(singleUnlimited?1:8)&&results.every(item=>(singleUnlimited||item.within20Minutes)&&item.pipelineVersion===CURRENT_PIPELINE_VERSION&&item.hardBudgetViolations===0&&item.platformOpen===0&&item.checksPassed&&item.unverifiedScopeIds.length===0&&['completed','needs-attention'].includes(item.status))};
